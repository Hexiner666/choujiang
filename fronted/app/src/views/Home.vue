...
<script>
import { prizeList } from '../assets/js/database'
...
export default {
  ...
  computed: {
    rotateStyle () {
      return `
        -webkit-transition: transform ${this.database.duration}ms ${this.database.mode};
        transition: transform ${this.database.duration}ms ${this.database.mode};
  ...
  methods: {
    ...
    rotating() {
      const { isRotating, angleList, database, rotateAngle, index } = this
